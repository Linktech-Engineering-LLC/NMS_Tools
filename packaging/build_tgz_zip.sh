...
STAGING_DIR="$SCRIPT_DIR/staging/nms_tools"
BIN_SRC="$REPO_ROOT/build/linux-x86_64"
#!/usr/bin/env bash
#
# build_tgz_zip.sh — Build portable TGZ/ZIP packages for NMS_Tools
#
...
(
    ...
    tar -czf "$OUTPUT_DIR/nms_tools-${VERSION}.tgz" nms_tools
)

echo "[NMS_Tools] Creating ZIP package..."
(
    cd "$SCRIPT_DIR/staging"
    zip -r "$OUTPUT_DIR/nms_tools-${VERSION}.zip" nms_tools >/dev/null
)

echo "[NMS_Tools] TGZ/ZIP packages created:"
echo " - $OUTPUT_DIR/nms_tools-${VERSION}.tgz"
echo " - $OUTPUT_DIR/nms_tools-${VERSION}.zip"
